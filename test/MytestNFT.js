...
describe("MyNFT", function () {
    ...
    it("Debe evitar que un usuario transfiera un NFT que no posee", async function () {
        const uri = "ipfs://exampleUri";
        await nft.mintNFT(owner.address, uri);
        await expect(nft.connect(addr1).transferFrom(owner.address, addr1.address, 0))
            .to.be.revertedWithCustomError(nft, "MaxSupplyRea"); 
    });
    
    it("Debe fallar si un usuario intenta mintear", async function () {
        await expect(nft.connect(addr1).mintNFT(addr1.address, "ipfs://exampleUri"))
            .to.be.revertedWithCustomError(nft, "NonexistentToken(uint256 tokenId)"); 
    });
